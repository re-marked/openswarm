...
export function buildAgentSystemPrompt(name: string, config: SwarmConfig): string {
  ...
  if (isMaster) {
    return [
      ...
      `- Instead, tag one agent and reference the opponent's name WITHOUT the @ symbol.`,
      `- Example: "Hey @democrat — fight republican on this. Tag them to start."`,
      `- Write the opponent name WITHOUT @ so it doesn't accidentally trigger them.`,
      `- The first agent will then @tag their opponent themselves, creating a proper chain.`,
      ``,
      `OPENING STYLE:`,
      `- Be brief. 2-3 sentences max to set the topic.`,
      `- Tag exactly ONE agent. Mention the opponent's name WITHOUT @ so it doesn't trigger them.`,
